...
Targets Java release: 10

How to build

- Build only this module (use toolchains if your JDK doesn't match):

  mvn -pl java10 -am -DskipTests verify

- To run the simple Main (if present):

  java -jar java10/target/java10-0.0.1-SNAPSHOT.jar

Appendix: Required JDK & toolchains

- Required JDK: Java 10
- Recommended: install Java 10 locally or configure `~/.m2/toolchains.xml` to point to a Java 10 JDK.

Examples:

```bash
# build only this module (command-line)
mvn -pl java10 -am -DskipTests clean package

# build using the profile added to the parent POM
mvn -Pbuild-java10 clean package
